...
def get_pipeline(base_name, bitrate, bitrate_max_multiplier, codec='hevc'):
...
@torch.no_grad
def rtn_nvenc_fwrd(model, dev, mlp_layer_bitrates, attn_layer_bitrates, bitrate_max_multiplier):
    '''
    From GPTQ repo
    ...
    for i in tqdm(range(len(layers)), desc="(RtN Quant.) Layers"):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                pipeline = get_pipeline(name, bitrate, bitrate_max_multiplier)
...
def process(model, mlp_layer_bitrates, attn_layer_bitrates, bitrate_max_multiplier):
    original_size, encoded_size, per_layer_encoded_size, per_layer_original_size = rtn_nvenc_fwrd(model, quarot_utils.DEV, mlp_layer_bitrates, attn_layer_bitrates, bitrate_max_multiplier)
...
if __name__ == '__main__':
    ...
    with torch.inference_mode():
        model, original_size, encoded_size, per_layer_encoded_size, per_layer_original_size = process(model, mlp_layer_bitrates, attn_layer_bitrates, args.bitrate_max_multiplier)
    model = model.cuda()
    print(model.model.layers[0].self_attn.q_proj.module.weight.sum())
