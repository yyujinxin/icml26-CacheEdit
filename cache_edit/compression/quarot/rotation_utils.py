...
from .hadamard_utils import random_hadamard_matrix, apply_exact_had_to_linear, is_pow2, matmul_hadU_cuda
try:
    from fast_hadamard_transform import hadamard_transform
except ImportError:
    print("Warning: hadamard_transform not available")
...
def fuse_ln_linear(layernorm: torch.nn.Module, linear_layers: typing.Iterable[torch.nn.Linear]) -> None:
    ...
    for linear in linear_layers:
        ...
        # linear.weight.data = (W_ * layernorm.weight.float()).to(linear_dtype)
        linear.weight.data = (W_ * layernorm.weight).to(linear_dtype)

        if hasattr(layernorm, 'bias'):
            if linear.bias is None:
                linear.bias = torch.nn.Parameter(torch.zeros(linear.out_features, dtype=torch.float))
            linear.bias.data = linear.bias.data.float() + torch.matmul(W_, layernorm.bias.float())
...
def rotate_embeddings(model, Q: torch.Tensor) -> None:
    ...
    for W in model_utils.get_embeddings(model, model_type):
        ...
        Q = Q.to(device=utils.DEV, dtype=torch.float32)
...
def rotate_attention_inputs(layer, Q, model_type) -> None:
    ...
    for W in [layer.self_attn.q_proj, layer.self_attn.k_proj, layer.self_attn.v_proj]:
        ...
        W_ = W.weight.to(device=utils.DEV, dtype=torch.float32)
        W.weight.data = torch.matmul(W_, Q).to(device="cpu", dtype=dtype)

class QKVRotWrapper(torch.nn.Module):
    def __init__(self, module:torch.nn.Linear, Q:torch.Tensor):
        super(QKVRotWrapper, self).__init__()
    ...
    def forward(self, x):
        weight = torch.matmul(self.weight.data, self.Q)
        return torch.nn.functional.linear(x, weight, self.bias)

def add_qkvrot(layer, Q, model_type) -> None:
    layer.self_attn.q_proj = QKVRotWrapper(layer.self_attn.q_proj, Q)
    layer.self_attn.k_proj = QKVRotWrapper(layer.self_attn.k_proj, Q)
    layer.self_attn.v_proj = QKVRotWrapper(layer.self_attn.v_proj, Q)

class RMSRotWrapper(torch.nn.Module):
    def __init__(self, module:transformers.models.llama.modeling_llama.LlamaRMSNorm, Q:torch.Tensor):
        super(RMSRotWrapper, self).__init__()
        assert isinstance(module, transformers.models.llama.modeling_llama.LlamaRMSNorm)
        self.module = module
        self.weight = module.weight
    ...
    def forward(self, hidden_states):
        ...
        return self.weight * torch.matmul(hidden_states.to(input_dtype), self.Q.T)

def add_rmsrot(layer, Q, model_type) -> None:
    layer.input_layernorm = RMSRotWrapper(layer.input_layernorm, Q)
...
def rotate_model(model, rotate_mode="hadamard"):
    ...
    for idx, layer in enumerate(tqdm.tqdm(layers, unit="layer", desc="Rotating")):
        ...
        add_rmsrot(layers[idx], Q, model_type)
...
class ActRotWrapper(torch.nn.Module):
    '''
    ...
    def __init__(self, module:torch.nn.Linear):
        super(ActRotWrapper, self).__init__()
        assert isinstance(module, torch.nn.Linear) or isinstance(module, QKVRotWrapper)
        self.module = module
        self.weight = module.weight
        self.bias = module.bias
...
def add_actrot(module, name='', layers=[torch.nn.Linear,
                                          ActRotWrapper,
                                          QKVRotWrapper,
                                          transformers.models.falcon.modeling_falcon.FalconLinear]):
    if isinstance(module, ActRotWrapper):
        return
    for attr in dir(module):
        tmp = getattr(module, attr)
        if type(tmp) in layers:
            setattr(module, attr, ActRotWrapper(tmp))
        if type(tmp) == torch.nn.Sequential:
            replaced = []
            for i, child in enumerate(tmp.children()):
                if type(child) in layers:
                    replaced.append(ActRotWrapper(child))
                else:
    ...
    for name1, child in module.named_children():
        add_actrot(child, name + '.' + name1 if name != '' else name1, layers)

def find_rlayers(module, layers=[torch.nn.Linear,
                                ActRotWrapper], name=''):
    if type(module) in layers:
        return {name: module}
    res = {}
    for name1, child in module.named_children():
        res.update(find_rlayers(
