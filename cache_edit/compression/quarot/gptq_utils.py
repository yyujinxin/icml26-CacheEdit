    ...
    def fasterquant(
        ...
        for i1 in range(0, self.columns, blocksize):
            ...
            for i in range(count):
                ...
                q_forward = self.quantizer.forward(w.unsqueeze(1))[0]
                q = self.quantizer.backward(q_forward).flatten()
                Q1[:, i] = q
                Losses1[:, i] = (w - q) ** 2 / d ** 2
...
@torch.no_grad()
def gptq_fwrd(model, dataloader, dev, nsamples, w_groupsize, percdamp, act_order):
    '''
    From GPTQ repo
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                gptq[name].fasterquant(
                    percdamp=percdamp, groupsize=layer_w_groupsize, actorder=act_order, static_groups=False
                )
...
def rtn_fwrd(model, dev):
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="(RtN Quant.) Layers"):
        ...
        for name in subset:
            ...
            if name not in qkv_name:
                quantizer.find_params(W)
                subset[name].weight.data = quantizer.quantize(W).to(
                    next(iter(layer.parameters())).dtype)
            else:
                quantizer.find_params(W.T)
                subset[name].weight.data = quantizer.quantize(W.T).T.to(
                    next(iter(layer.parameters())).dtype)
...
@torch.no_grad()
def rtn_nvenc_fwrd(model, dev, kv_pipeline, q_pipeline, o_pipeline, mlp_up_pipeline, mlp_down_pipeline):
    '''
    From GPTQ repo
    TODO: Make this function general to support both OPT and LLaMA models
    '''
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="(RtN Quant.) Layers"):
        layer = layers[i].to(dev)
        full = quant_utils.find_qlayers(layer, layers=[torch.nn.Linear])
        for names, pipeline in zip(sequential, pipelines):
            subset = {n: full[n] for n in names}
            for name in subset:
