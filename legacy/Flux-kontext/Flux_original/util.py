...
def save_image(
    ...
    if nsfw_score < nsfw_threshold:
        ...
        print(f"img.shape: {x.shape}, nsfw_score: {nsfw_score:.4f}")
        if track_usage:
...
configs = {
    ...
    "flux-dev-kontext": ModelSpec(
        repo_id="/data1/model/FLUX.1-Kontext-dev",
...
def load_flow_model(name: str, device: str | torch.device = "cuda", verbose: bool = True) -> Flux:
    ...
    ckpt_path = "/data1/model/FLUX.1-Kontext-dev/flux1-kontext-dev.safetensors"
...
def load_t5(device: str | torch.device = "cuda", max_length: int = 512) -> HFEmbedder:
    ...
    return HFEmbedder("/data1/model/google-T5", max_length=max_length, torch_dtype=torch.bfloat16).to(device)


def load_clip(device: str | torch.device = "cuda") -> HFEmbedder:
    return HFEmbedder("/data1/model/openai/clip-vit-large-patch14", max_length=77, torch_dtype=torch.bfloat16).to(device)
...
def load_ae(name: str, device: str | torch.device = "cuda") -> AutoEncoder:
    ...
    ckpt_path = "/data1/model/FLUX.1-Kontext-dev/ae.safetensors"
