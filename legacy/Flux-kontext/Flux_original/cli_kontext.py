...
def parse_img_cond_path(options: SamplingOptions | None) -> SamplingOptions | None:
    ...
    while True:
        ...
        print("img_cond_path: ", img_cond_path)
        if img_cond_path.startswith("/"):
            if img_cond_path.startswith("/home"):  
...
def main(
    ...
    while opts is not None:
        ...
        if opts.guidance is None:
            ...
            opts.guidance = 16
        opts.guidance = 16
        print(f"Generating with seed {opts.seed} and guidance {opts.guidance}:\n{opts.prompt}")
        ...
        # print(f"x_unpack.shape: {x.shape}")   torch.Size([1, 16, 146, 110])
